...
namespace Ceylan
{
	...
	namespace Network
	{
		...
		class CEYLAN_DLL HostDNSEntry : public Ceylan::TextDisplayable
		{
			 ...
			 * to system host entry, used to avoid including system-specific
			 * headers such as netdb.h.
			 *
			...
			public:
				 ...
				 *
				 * @param hostName the name of host to create a DNS description
				 * for.
				 *
				 * @throw NetworkException if the operation is not supported on
				 * the platform, or is supported but failed.
				 *
				 ...
				 *
				 * @note Ownership of the list is transferred to the caller, who
				 * will have to deallocate it when finished with it.
				 *
				 */
				 ...
				 *
				 * @note It is actually the type of the official address, since
				 * there can be several addresses of several types in the
				 * address (alias) list.
				 *
				 */
				...
				/**
				 * Deletes properly the data-structure returned by getAddresses.
				 *
				 * (helper function)
				 *
				 */
				static void DeleteAdresses(
		 ...
		 *
		 * @note The user needs to have special permissions to change the host
		 * name.
		 *
		 * @throw NetworkException if the renaming failed, including if the
		 * operation is not supported on the running platform.
		 *
		 */
		 ...
		 *
		 * @return a string containing the domain name, or an empty string if no
		 * appropriate name is found. For example, on Linux systems, if NIS is
		 * not in use, this method reads the content of
		 ...
		 *
		 * Tries to gather most useful information about host name, including
		 * FQDN.
		 *
		 ...
		 *
		 * @note Reverse name look-up thanks to IP address could be performed
		 * too.
		 *
		 */
		 ...
		 * Returns the fully qualified domain name (FQDN) of specified host.
		 *
		 * @param hostName the name of host to create a DNS description for. It
		 * is a host name (ex: "esperide.com"), not a stringified IP address.
		 *
		 */
		...
		/**
		 * The numerical error code that may be returned by specific network
		 * primitives, namely Windows socket ones.
		 *
		 */
		...
		/**
		 * Returns a string describing the error associated with the specified
		 * error code.
		 *
		...
		/**
		 * The network manager takes care of the initialization and shutdown of
