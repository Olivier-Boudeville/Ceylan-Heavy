...
namespace Ceylan
{
	...
	namespace Network
	{
		...
		/**
		 * Opaque handle for forward-declared but undefined struct pointer to
		 * system socket address, used to avoid including system-specific
		 * headers which define for example sockaddr_in.
		 *
		 * Otherwise the API exposed by Ceylan would depend on these headers,
		 * then on a config.h that should then be installed but may clash with
		 * others, and so on.
		 *
		 */
		 ...
		 * sockets and packet-based (datagram) sockets, both for client-side and
		 * server-side sockets.
		 *
		...
		class CEYLAN_DLL Socket : public System::InputOutputStream
		{
			...
			public:
				...
				/**
				 * Tells whether there is data available on input, i.e.  on the
				 * file descriptor used for transport.
				 *
				 */
				 ...
				 * written to this socket. Its size must be at least maxLength
				 * bytes.
				 *
				 * @return The number of bytes actually written, which should be
				 * equal to maxLength.
				 ...
				 * FeatureNotAvailableException if the file descriptor feature
				 * is not available.
				 *
				 ...
				 * server-side, it may use a different file descriptor from the
				 * original one, this method tells which one is to be used to
				 * read or write data.
				 *
				 * @throw SocketException if the operation failed, or
				 * FeatureNotAvailableException if the file descriptor feature
				 ...
				 *
				 * @note It is not an obvious question, since computers may have
				 * more than one interface, and often the socket are bound
				 * thanks to INADDR_ANY, which means any interface.
				 *
				 * @throw SocketException if this operation failed.
				 *
				 */
				 ...
				 * StreamID.
				 *
				 * @throw InputStreamException if no identifier is available, or
			...
			protected:
				 ...
				 *
				 * @param blocking tells whether this socket should be created
				 * in blocking mode (the default) or in non-blocking mode.
				 *
				 ...
				 * FeatureNotAvailableException if the network feature is not
				 * available.
				 *
				 ...
				 * @throw SocketException if the operation failed.
				 *
				 * @note It is the place where socket attributes are effectively
				 * set (ex: non-blocking).
				 *
				 */
				 ...
				 * if false set to non-blocking mode. If the socket is already
				 * in the target state, nothing is done.
				 *
				 ...
				 *
				 * It will be used by server sockets to bind to the target port,
				 * whereas client sockets will use it to connect to the target
				 * server, and anonymous sockets will store informations about
				 * the client.
				 *
				 */
				 ...
				 * Depending on the specialization of this socket, it may or may
				 * not be the file descriptor that is used for transport.
				 *
				 * @note The Windows SOCKET datatype is mapped here to a file
				 * descriptor.
				 *
				 */
