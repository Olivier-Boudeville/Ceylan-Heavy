...
/**
 * This module offers the operators lacking in C++ in order to perform appending
 * of numerical values to standard strings, and basic string conversion.
 *
 * Various conversions are provided, from numerical to string and the other way
 * round.
 *
 * Each operator has its counterpart, which allows to add a numerical value to a
 * string in both orders: 'numerical + string' and also 'string + numerical'.
 *
 * Warning: a C++ compiler evaluates '"Four means " + 4' as 'char *' + 'int' so
 * it does pointer arithmetic and returns unexpected bytes: actually, it returns
 * the 'char *' starting with the space before "means", 4 characters after the
 * initial F.
 *
...
/*
 * No serialization of Uint64 and Sint64 for the moment, since on some platforms
 * they are just fake datatypes.
 *
...
namespace Ceylan
{
	...
	/**
	 * Returns a user-friendly textual representation of a bool ("true" or
	 * "false").
	 *
	 */
	 ...
	 *
	 * @param bitField if true, returns a binary representation of the specified
	 * value, prefixed by '0b'.
	 *
	 */
	...
	/**
	 * Converts the numerical value to a string with the value in hexadecimal
	 * format.
	 *
	 * @param value the value to convert in hexadecimal.
	 *
	 * @param prefix tells whether the hexadecimal prefix ("0x") should be
	 * added.
	 *
	 * @param minDigits tells which minimal number of digits should be put in
	 * the returned string (prefix not included). Leading zeroes are added to
	 * reach that number.
	 *
	 */
	...
	/**
	 * Converts the numerical value to a string, with specified precision, in
	 * fixed (not scientific) notation.
	 *
	 * @param value the value to convert to text.
	 *
	 * @param precision the requested precision, i.e. the number of digits that
	 * is to be displayed after the dot.
	 *
	 ...
	 * interpreted as a numerical value, and not as a character, as it would
	 * happen if toString was used.
	 *
	...
	/**
	 * Converts, if possible, a string containing an integer number into that
	 * number.
	 *
