	...
	namespace System
	{
		...
		class CEYLAN_DLL Stream : public Ceylan::TextDisplayable
		{
			...
			public:
				...
				/**
				 * Exception thrown when a stream is set to non-blocking whereas
				 * it failed or is not supported at all.
				 *
				 */
				 ...
				 *
				 * @param blocking tells whether this stream should be created
				 * in blocking mode (the default) or in non-blocking mode (if
				 * supported).
				 *
				 */
				...
				/**
				 * Tells whether this stream is in blocking mode (if true), or
				 * in non-blocking mode (if false).
				 *
				 */
				 ...
				 *
				 * It is passed by address so that this function can set it to
				 * zero on successful close.
				 *
				 * @return true iff an operation had to be performed.
				 *
				 * @throw CloseException if the close operation failed.
			...
			protected:
				 ...
				 * if false set to non-blocking mode. If the stream is already
