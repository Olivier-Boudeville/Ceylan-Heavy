	...
	namespace System
	{
		...
		class CEYLAN_DLL Console : public Ceylan::TextDisplayable
		{
			...
			public:
				 ...
				 *
				 * On Nintendo DS, it implies it will take the full extent of
				 * the bottom LCD screen and will provide a text area of 32x24
				 * characters. As a side-effect, the sub 2D core will be powered
				 * on, the console will use the second (sub) core, here located
				 * at the bottom actual screen, and using the VRAM bank C, the
				 * background #0 and the default font. Only available for the
				 * ARM9.
				 *
				 * @param startInForeground tells whether the console is to
				 * start in foreground (thus directly taking control of the its
				 * display and input devices, i.e. keys) or if it should be on
				 * the background, storing text non-interactively until it is
				 * set to foreground.
				 *
				 ...
				 * debugging.
				 *
				 * @param startingX the abscissa of the top-left corner of the
				 * rectangle used for text output.
				 *
				 * @param startingY the ordinate of the top-left corner of the
				 * rectangle used for text output.
				 *
				 * @param width the width allowed for text output, starting from
				 * startingX.
				 *
				 ...
				 *
				 * @param useSubCore tells whether the main or the sub 2D engine
				 * should be used (default: sub core).
				 *
				...
				/**
				 * Sets the console to foreground if needed, render its text and
				 * enter its event loop so that the user can use the keys to
				 * browse its content.
				 *
				 * Key controls are:
				 ...
				 * change the video or input settings, the caller is expected to
				 * set them as wished after the call.
				 *
				 * @throw ConsoleException if the operation failed.
				 *
				 */
				...
				/**
				 * Renders in console output the current text of its character
				 * buffer.
				 *
				 * @throw ConsoleException if the operation failed.
				 *
				 ...
				 *
				 * @param durationBetweenRepeats duration in milliseconds before
				 * two key repeats are triggered, once they are repeating.
				 *
				 ...
				 *
				 * This method performs action only on the ARM9 of the Nintendo
				 * DS.
				 *
				 * @param useBottomScreen tells whether the bottom or the top
				 * physical screen should be used (default: bottom screen).
