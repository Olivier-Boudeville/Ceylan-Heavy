 ...
 *
 * Some file and directory operations are made available here, so that they are
 * virtual methods (able to be overriden in FileSystemManager child classes)
 * rather than static methods (non-overridable) in classes File, Directory,
 * etc. (filesystem implementations may have to define them specifically).
 *
 * But for the ease of programming, these FileSystemManager virtual methods will
 * be made available through File and Directory static methods, in charge of
 * selecting automatically the correct filesystem implementation and calling its
 * corresponding virtual method.
 *
 * This allows the user programs to call only File::MyMethod instead of calling
 * something like GetDefaultFileSystemManager().myMethod or, worst,
 * GetStandardFileSystemManager().myMethod: less readable, more error cases to
 * manage, and the user code would not be cross-platform.
 *
 * The current organization is to centralize in FileSystemManager child classes
 * the non-static implementations of all the methods that would be static for a
 * given file or directory child class: these implementations must not be static
 * (so that they are overridable) hence cannot be defined directly from File and
 * Directory abstract classes (otherwise polymorphism could not be used, and
 * these two abstract classes would have to be tightly coupled to their child
 * classes). Obviously non-static methods are to be called from instances that
 * can be neither File nor Directory instances.
 *
 */
...
list<string> FileSystemManager::splitPath( const string & path )
{
	...
	/*
	 * Not wanting to use in-out argument such as 'splitPath( list<string> &
	 * nodes ...)', we have to return it. We are therefore unable to return
	 * directly the number of elements.
	 *
	 */
	...
	/*
	 * With an initial value of false, "/mnt/..." would lead to [ 'mnt', '...' ]
	 * instead of the expected [ '', 'mnt', '...' ].  Doing so is needed to make
	 * a difference between "/mnt/..." and "mnt/...".
	 *
	 */
	...
	bool deallocatePreviousIfAny )
{
	...
	if ( deallocatePreviousIfAny )
	{
		 ...
		 * class-specific variable, its destructor must perform every needed
		 * unsubscribing by itself.
		 *
		 */
