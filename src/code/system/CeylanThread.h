...
namespace Ceylan
{
	...
	namespace System
	{
		...
		class CEYLAN_DLL Thread: public Runnable
		{
			 ...
			 * to system thread identifier, used to avoid including
			...
			/**
			 * Opaque handle for forward-declared but undefined struct pointer
			 * to system thread attribute, used to avoid including
			 * system-specific headers such as pthread.
			 *
			 * Otherwise the API exposed by Ceylan would depend on these
			 * headers, then on a config.h that should then be installed but may
			...
			/**
			 * Opaque handle for forward-declared but undefined struct pointer
			 * to system condition variable, used to avoid including
			 * system-specific headers such as pthread.
			 *
			 * Otherwise the API exposed by Ceylan would depend on these
			 * headers, then on a config.h that should then be installed but may
			...
			public:
				...
				/**
				 * Raised when POSIX returns thread errors, mainly in creation.
				 *
				 */
				 ...
				 *
				 * It creates a new POSIX thread which calls the method start().
				 *
				 * @example:
				 * <pre>
				 ...
				 * </pre>
				 *
				 * @throw RunnableException if the thread could not be run
				 * properly, including if the multheading feature is not
				 * supported.
				 *
				 */
				virtual void run() ;
				 ...
				 * Hence this method does not make the thread stop, it does ask
				 * it to stop in the hope it will be heard: if the thread does
				 * not invoke the stopDemanded() method systematically, it will
				 * never know about this request.
				 *
				 *
				...
				/**
				 * Suspends the calling thread until the instance's thread has
				 * stopped.
				 *
				 * @example:
				 * <pre>
				 ...
				 *
				 * @param microseconds the number of microseconds to wait, must
				 * be less than one second, it is only the remainder of the
				 * total wanted duration.
				 *
				...
				class CEYLAN_DLL Waiter : protected Mutex
				{
					...
					public:
						 ...
						 * multithreading feature is not available, so that no
						 * Waiter instance can be created in this case.
						 *
						 * @see Feature::isMultithreadingSupported
						 *
						 */
						 ...
						 * until time is elapsed, or signal() or broadcast() is
						 ...
						 * during the requesting time. Always returns true if
						 * the multithreading feature is not available.
						 *
						 */
						 ...
						 * returns true if the multithreading feature is not
						 * available.
						 *
						 */
						bool signal() ;
					...
					private:
						...
						/**
						 * This Waiter's system-specific condition variable,
						 * since multiple threads can wait on a single Waiter.
						 *
						 */
			...
			protected:
				 ...
				 *
				 * This method must be wrapped by a subclass method to treat the
				 * critical behaviour.
				 *
			...
			private:
				 ...
				 *
				 * These information can be read or written from different
...
#pragma warning( disable : 4251 )
				...
				/**
				 * Tells whether this thread must stop, i.e. whether it has been
				 * requested to do so.
				 *
				 */
