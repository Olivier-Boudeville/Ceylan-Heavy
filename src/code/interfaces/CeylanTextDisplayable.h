...
namespace Ceylan
{
	...
	/**
	 * Interface that every Displayable able to show a textual representation of
	 * itself should implement.
	 *
	 * @note Copy constructor and assignment operator cannot be private, because
	 * some TextDisplayable instances need to rely on their copy constructor,
	 * for example for operators returning them by value.
	 *
	 * @note The output format (raw text or HTML format) is retrieved from
	 * overall settings, so that it can be easily chosen at runtime, for example
	 * thanks to the LogPlug.
	 *
	 * @see Displayable.
	 *
	 */
	class CEYLAN_DLL TextDisplayable : public Displayable
	{
		...
		public:
			...
			/**
			 * Returns a user-friendly description of this list of pointers to
			 * text displayable instances.
			 *
		...
		private:
			...
			/**
			 * Assignment operator should not be private since it is useful in
			 * some cases (ex: Vector instances).
			 *
