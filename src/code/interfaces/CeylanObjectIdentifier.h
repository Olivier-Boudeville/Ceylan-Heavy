...
namespace Ceylan
{
	...
	namespace Log
	{
		 ...
		 * identifier, and allows smart aggregation, when messages sent from 
		 * a object constructor, therefore with a wrong class name, are 
		 * conveyed in their right log channel, thanks to the address matching.
		 *
		...
		class CEYLAN_DLL ObjectIdentifier : public TextIdentifier
		{
			...
			public:
				 ...
				 *
				 * @note Use that constructor when object is not available, 
				 * such as when being on log aggregator's side.
				 *
				 */
				 ...
				 * Loggable instance, since messages sent from their 
				 * constructor have a faulty class name (mangled).
				 *
				 * A risk of mistake arises with automatic variables, since,
				 * as they are allocated on the stack, they can easily be at 
				 * the same address.
				 *
				 * This method is notably used by smart aggregators so that 
				 * the corresponding messages are classified in the right
				 * channels.
				 *
				 */
				 ...
				 * channel name, which would probably mean that this channel
				 * name does not correspond to the private channel of an
				 * object. 
				 *
				 * @note Ownership of the identifier is transferred to the
				 * caller.
				 *
				 * If an exception is raised, no prior ObjectIdentifier
				 * has been instanciated, so there will not be memory leaks.
				 *
				 */
			...
			protected:
				...
				/**
				 * The address of the referenced object in process memory 
				 * space.
				 *
				 */
				const void * _address ;
