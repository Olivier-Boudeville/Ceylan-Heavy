...
namespace Ceylan
{
	 ...
	 * It is especially useful in order to compare for equality two objects, since the probability
	 * they have the same hash code should be low.
	 *
	 */
    typedef Uint32 StrongHashCode ;


    /**
     * Interface that every object able to compute a hash value should implement, to ease
     * hashtables handling.
     *
	 * @note It should be said whether the hashcodes depend on the state of the Hashable.
	 * At least in theory, they should not.
	 *
     * @see Object.
     *
     */
    class Hashable : public TextDisplayable
    {
        ...
        public:
             ...
             *
             */
			virtual const std::string toString( VerbosityLevels level = high ) const throw() ;


			/// Returns the weak hash code corresponding to the specified string.
			static WeakHashCode GetWeakHashCode( const std::string & stringToHash ) throw() ;
			
			/// Returns the strong hash code corresponding to the specified string.
			static StrongHashCode GetStrongHashCode( const std::string & stringToHash ) throw() ;
