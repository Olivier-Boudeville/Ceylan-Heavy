...
namespace Ceylan
{
	...
	namespace Log
	{
		 ...
		 * in chronological order. Associated command-line option:
		 * '--classicalPlug'
		 *
		 * - HTMLPlug: all logs are output in a set of HTML files gathered by a
		 * generated frameset, where each log channel has a dedicated HTML
		 * page. Associated command-line option: '--HTMLPlug'
		 *
		 ...
		 * variable in main() functions, so that it is automatically in charge
		 * of the Log system's life cycle.
		 *
		 * Besides, it allows to select the log plug to use at run-time, thanks
		 * the command-line plug options that may be passed to the executable
		 * and, therefore, to the LogHolder instance.
		 *
		 * If this program has been called with '--HTMLPlug' as parameter in the
		 * command line, then the HTML log plug with be selected instead of the
		 * classical raw text plug ('--classicalPlug') or the console plug
		 * ('--consolePlug'), and log messages will be output accordingly.
		 * Choosing '--nullPlug' will result in no logs being output.
		 *
		 ...
		 * destruction might result in log messages. Thus no operation on a
		 * LogHolder instance should result in an exception, instead an
		...
		class CEYLAN_DLL LogHolder : public TextDisplayable
		{
			...
			public:
				 ...
				 * Creates a LogHolder.
				 *
				 * Reads the command-line arguments, and uses them to choose a
				 * specific LogPlug, if specified, otherwise uses default log
				 * plug.
				 *
				 * Command-line parameters are left untouched (read-only).
				 *
				 * Only the first log plug specification found in command-line
				 ...
				 *
				 * @param immediateWrite if true, selects immediate write for
				 * the selected plug, if supported (this is the case of the
				 * console and the classical plug, but not for the HTML one);
				 * otherwise (if false), deferred writes will be performed (if
				 * the plug supports it).
				 *
				 * @note There is no point in using more than one instance of
				 * LogHolder in a process.
				 *
				 * @throw LogException if the log system could not be correctly
				 * started.
				 *
				 */
				...
				/**
				 * Returns whether the specified option is a known plug option.
				 *
				...
				/**
				 * Command-line option which is to be used to select the console
				 * plug.
				 *
				 */
				...
				/**
				 * The default plug, chosen if no specific plug is requested
				 * thanks to command-line parameters.
				 *
				 */
