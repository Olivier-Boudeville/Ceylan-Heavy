...
/*
 * In the case of the Nintendo DS, a convenient way of debugging is to use one
 * of the two screens as a debug console.
 *
 * To allow to look at all logs, even if they are produced quickly, a console
 * (CeylanConsole) is used. It is created as soon as the LogAggregatorConsole is
 * created, and, when this instance is deleted, it gives the opportunity to
...
void LogAggregatorConsole::aggregate()
{
	...
	/*
	 * Even in non-immediate write mode, nothing special is to be done here, as,
	 * on the DS, messages have already been collected in the text buffer of the
	 * console.
	 *
	 */
	return ;
...
void LogAggregatorConsole::store( LogMessage & message )
{
	...
	/*
	 * On the DS ARM9, with immediate write or not, messages are to be stored in
	 * the console text buffer, that will display them at once (if set to
