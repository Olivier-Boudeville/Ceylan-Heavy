...
LogChannel * LogAggregator::findChannel( const string & channelName ) const
{
	...
	/*
	 * Two cases: we have got to search a LogChannel or an ObjectChannel,
	 * depending on the message being aimed at a Loggable channel or not.
	 *
	 */
...
void LogAggregator::transferChannel( LogChannel & source, LogChannel & target )
{
	...
	/*
	 * For each message of source channel, update its channel and move it to the
	 * target channel.
	 *
	 */
		...
		it != source._messages.end(); it++ )
	{
		...
		/*
		 * Ownership transferred to target channel, message pointers set to null
		 * to avoid possible double deallocation of transferred messages.
		 *
		 * (should be useless)
		 *
		 */
	...
	/*
	 * After having nullified all message pointers in source list, remove these
	 * null pointers:
	 *
	 */
...
void LogAggregator::store( LogMessage & message )
{
	...
	/*
	 * Tests whether this message is an object one or not, and manages it
	 * accordingly:
	 *
	 */
...
void LogAggregator::createBasicChannelFrom( LogMessage & message )
{
	...
	/*
	 * Simply, creates the new channel and stores its first message in it:
	 *
	 */
...
void LogAggregator::createLoggableChannelFrom( LogMessage & message )
{
	...
	/*
	 * We are therefore here in a smart aggregator which will fix mangled
	 * Loggable names.
	 *
	 */
	...
	/*
	 * First, check that this incoming channel name corresponds to an object
	 * channel.
	 *
	 * We may be in the case where previous messages had been sent with the
	 * mangled class name because of the Object constructor.
	 *
	 * Let's seek whether it is really the case and, if so, let's move those
	 * messages to their right loggable channel (i.e. the new one, with a
	 * non-mangled class name).
	 *
	 */
	...
	/*
	 * This message corresponds that an object channel which does not exist
	 * already.
	 *
	 * Second, scan existing channels to know whether there is one whose
	 * instance address matches: it would have been created with a mangled class
		...
		it != _channelList.end(); it++ )
	{
		...
		if ( objChannel )
		{
			...
			/*
			 * This element of the list is an ObjectChannel, maybe which had the
			 * mangled name?
			 *
			 */
			...
			if ( identifierFromMessage->differentButMatches( * id ) )
			{
				...
				/*
				 * Yes, so transfer the messages after having created the
				 * channel:
				 *
				 */
				...
				/*
				 * Do not forget to add this incoming message which triggered
				 * the transfer!
				 *
				 */
				newChannel.addMessage( message ) ;
	...
	/*
	 * We went through the list and did NOT find anything, so let's create it in
	 * the next step, which is common with non-smart aggregators.
	 *
	 */
...
void LogAggregator::storeObjectMessage( LogMessage & objectLogMessage )
{
	...
	if ( channel == 0 )
	{
		...
		/*
		 * Object Channel not found. A channel is to be created, a brand new or
		 * an unmangled form replacing a former object channel.
		 *
		 * Either this is a first message sent to a new object channel, or this
		 * is the first unmangled message sent from a previously mangled object
		 * channel.
		 *
		 */
	...
	/*
	 * Yes, object channel already available, let's use it and add the incoming
	 * message.
	 *
	 */
	...
	/*
	 * Removing now useless protocol prefix of this object message's header:
	 *
	 */
	...
	LevelOfDetail level )
{
	 ...
	 * MaximumLevelOfDetailForMessage:' cannot refer to an extern, in C/C++ No
	 * comment.
	 *
	 */
	 ...
	 * top-priority messages are taken into account, hence the verbosity is low,
	 * etc.
	 *
	 */
