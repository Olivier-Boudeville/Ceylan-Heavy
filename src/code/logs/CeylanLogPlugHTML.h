...
namespace Ceylan
{
	...
	namespace Log
	{
		 ...
		 * instances of the log framework in their HTML version, and creates the
		 * relevant standard channels.
		 *
		 * With this Log implementation, log informations will be output in HTML
		 * format.
		 *
		 ...
		 * are lost, but on the other hand logging will induce very low runtime
		 * overhead (processing-wise; memory-wise this can be another story), so
		 * this kind of logs can be useful to debug time-critical operations
		 * (ex: a scheduler).
		 *
		 * A web page will correspond to each log channel and list its messages.
		 *
		 * All web pages will be gathered by an HTML frameset allowing to browse
		 * channels by picking their names.
		 *
		...
		class CEYLAN_DLL LogPlugHTML : public LogPlug
		{
			...
			public:
				 ...
				 *
				 * @param plugInitiator the name of the speaker, for instance
				 * argv[0].
				 *
				 * @throw LogException if the service could not be started.
				 *
				 * @note plugInitiator will be used to create the directory
				 * which will be containing all HTML log output.
				 *
				 * @param smart tells whether the HTML aggregator should be
				 * smart.
				 *
				 */
			...
			protected:
				 ...
				 /**
				  * The suffix that will be added to <b>sourceName</b> to define
				  * the log directory.
				  *
				  */
