...
namespace Ceylan
{
	...
	namespace Log
	{
		 ...
		 * instances of the log framework in their raw version, and creates the
		...
		class CEYLAN_DLL LogPlugConsole : public LogPlug
		{
			...
			public:
				 ...
				 * argv[0].
				 *
				 * @param immediateWrite tells whether the console aggregator
				 * should store messages as soon as they arrive, or wait till
				 * asked before aggregating them.
				 *
				 * @param smart tells whether the console aggregator should be
				 * smart.
				 *
				 * @note Since log messages will be output to a file named
				 * 'sourceName'.log, sourceName must be a valid prefix file
				 * name.
				 *
				...
				/**
				 * Returns some informations about the console LogPlug's state.
				 *
				 */
