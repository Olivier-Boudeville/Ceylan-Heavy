...
run() ->
	...
	case init:get_argument('-batch') of
		...
		_ ->
			% Once 'Enter' is hit, will go back to the current shell instead
			% of going one with the next lines, probably because we are running
			% this module thanks to '-eval': 
			io:get_line( ?Prefix 
				"Running in interactive mode, not terminating "
