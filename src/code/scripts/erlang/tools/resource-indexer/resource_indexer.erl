...
main() ->
	...
	Files = file_utils:find_files_from( ScanDir ),
...
write_header_file_entries_header( File, DefineSymbol ) ->
	...
	io:format( File, "/* ~n", [] ),
	io:format( File, " * File generated by resource_indexer.erl, the Ceylan~n",
		[] ),
	io:format( File, " * resource indexer script.~n", [] ),
	io:format( File, " *~n", [] ),
...
write_header_file_entries( File, [H|T] ) ->
	io:format( File, "  static const Ceylan::ResourceID ~s = ~B ;~n",
