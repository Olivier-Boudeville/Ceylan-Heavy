...
% Run the tests in interactive mode, no prior TimeManager expected to be alive.
run() ->
	...
	receive
		...
		{beginTick,SecondTick} ->
			?test_info([ io_lib:format( "Received second top: ~B.", 
				[ SecondTick ] ) ]),	
			?test_info([ "Second top will be acknowledged late on purpose "
				"by the test to the manager (sleeping now)." ]),
			timer:sleep( 2000 ),
