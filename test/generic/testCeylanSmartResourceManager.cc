...

  {
	...
	try
	{
	  ...
	  // Indented block to force premature manager deallocation:
	  {
		...
		/*
		 * Check a special case of drop, when two resources are submitted with
		 * the same key:
		 *
		 */
		bool raised = false ;
		...
		try
		{
		  ...
		  cannotReplace = new TestSmartResource( "cannotReplace" ) ;
		...
		LogPlug::info( "Deleting this clone should not interfere "
		  "with this manager." ) ;
		delete firstClone ;
