...
public:
  ...
  void run() throw( ServerStreamSocketException )
  {
	...
	/*
	 * Instead of relying on a connection count to know when to stop, the order
	 * could come from the network as well, with for example a bool
	 * _requestedToStop:
	 *
	...
	throw( ServerStreamSocketException )
  {
	...
	/*
	 * newConnection does not have to be used, since read/write methods are also
	 * available for sequential servers (only one client can exist, these
	 * methods are linked to it)
	 *
	 */
