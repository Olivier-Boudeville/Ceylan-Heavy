...
int main( int argc, char * argv[] )
{
    ...
    try
    {
...

		{
			...
			/*
			 * This example is made to understand how automatic variables
			 * can be mixed up.
			 *
		 ...
		 *
		 * This is a test to show that Log system is well managed, i.e. 
		 * that it at least tries to aggregate and save the log in case 
		 * something went wrong.
		 * 
		 * One can test various stop conditions by selecting the appropriate
		 * one, building and running the test and finally by checking whether
		 * the logs are really available.
		 *
		 * The LogHolder is used in all cases.
		 * 
		 * The 'StopOnException' case is meant to deal with correctly 
		 * managed exceptions (not unexpected ones, i.e. whenever the raised
		 * exception is not listed in an explicit exception specification).
		 *
		...
		/*
		 * The result depends on the log plug being used and the stop
		 * condition.
		 *
		 ...
		 *		 
		 *  - with the console plug : nothing can be lost in any stop
		 * condition, even if redirected (ex : 'myProgram 1> logs.txt 2>&1')
		 *
		 ...
		 *		+ StopOnDivisionByZero : 
		 *	 
		 * Nevertheless the  plugs can be safe (not loosing any log) by
		 * setting their write mode to 
		 *
		 *  - with the HTML plug :
		 *		+ StopOnException : all logs are lost
		 *
		 * Any stop condition other than 'NormalStop' will lead to label this
		 * test as failed.
		 *
		 */
