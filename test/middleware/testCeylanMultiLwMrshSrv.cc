...
public:
	...
	throw( MultiplexedServerStreamSocketException )
  {
	...
	/**
	 * Not really proper design: a marshaller is created each time a connection
	 * has available data!
	 *
	 * What should be done is a connection-tracking, so that one and only one
	 * marshaller is associated with each selected connection, i.e. an
	 * AnonymousProtocolAwareStreamSocket should be used here.
	 *
	 * @see testCeylanMultiLwProtocolServer that implements that and more.
	 *
	 *
	 */
	...
	else
	{
	  ...
	  /*
	   * However we will not do anything with it here, and kill the socket.
	   *
	   */
...
private:
   ...
   *
   * As it is a state-less marshaller, the same instance could be used by a set
   * of connections. If it was stateful, then each connection would have to be
   * associated with its own marshaller instance.
   *
   * However a marshaller depends on a specific I/O stream (connection), hence
   * as there is only one marshaller here, at most one connection at a time can
   * exist.
   *
   */
