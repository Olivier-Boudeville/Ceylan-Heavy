 ...
 *
 *       + ID #2: it is a fixed-length requests, 80 bytes have to be read, the
 * (possibly overflowed) sum of all these bytes, stored in one byte, is
 * returned. Connection is still alive afterwards.
 *
 *       + ID #3: it is an reversed-echo request, first two bytes are the length
 * of the sent string (Uint16), then the string itself. The string that is sent
 * back is the mirrored one. Connection is still alive afterwards.
 *
 ...
 *
 * This server is triggered by a protocol-aware connection socket, that will
 * call its notifyDataAvailability method.
 *
...
public:
	...
	throw( ProtocolException )
  {
	...
	/*
	 * Changing the internal state here is useless but clearer:
	 *
	 */
	_currentState = 1 ;
	...
	LightWeightMarshaller & marshaller ) throw( ProtocolException )
  {
	...
	/*
	 * We need 80 bytes, actually the buffer size has been chosen so that it
	 * cannot be achieved in one call, let's wait for it: (as the next 80-byte
	 * call will fit in buffer, no need to perform partial sums)
	 *
	 */
	...
	LightWeightMarshaller & marshaller ) throw( ProtocolException )
  {
	...
	/*
	 * Echoes strings can be more than 80-byte long, we need intermediate
	 * storage:
	 *
	 * (we cannot use LightWeightMarshaller::decodeString here since we cannot
	 * know for sure whether all necessary bytes are already available)
	 *
	 */
...
private:
   ...
   *
   * Here a state number corresponds roughly to the ID of the request being
   * processed.
   *
...
/**
 * This test server is multiplexed and uses light-weight marshalling thanks to a
...
public:
	...
	throw( ServerStreamSocketException )
  {
	...
	/*
	 * Let's create the server-specific protocol manager which will use the
	 * specified marshaller to take care of lower-level data conversion:
	 *
	 */
	...
	/*
	 * The socket takes ownership of the protocol server that itself owns the
	 * marshaller. Hence when the connection will be over, the
	 * connection-dedicated socket will be deallocated, which in turn will
	 * delete the protocol server, which in turn will delete the marshaller.
	 *
	 */
...
private:
   ...
   *
   * As it is a state-less marshaller, only one instance can be used by a set of
   * connections. If it was stateful, then each connection would have to be
   * associated with its o
   *
   */
...
/**
 * Test of Ceylan light-weight marshalling in the context of a multiplexed
 * server.
 *
...
int main( int argc, char * argv[] )
{

  {
	...
	try
	{
	   ...
	   * returning, so that it lets enough time for the client to stop and then
	   * to wait for the server PID.
	   *
