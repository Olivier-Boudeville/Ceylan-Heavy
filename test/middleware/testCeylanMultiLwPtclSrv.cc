...
class MyProtocolServer : public Ceylan::Middleware::ProtocolServer
{
	...
	public:
			...
			LightWeightMarshaller & marshaller ) throw( ProtocolException )
		{
			 ...
			 * that it cannot be achieved in one call, let's wait for it: (as
			 * the next 80-byte call will fit in buffer, no need to perform
			 * partial sums)
			 *
			 */
			...
			LightWeightMarshaller & marshaller ) throw( ProtocolException )
		{
			 ...
			 *
			 * (we cannot use LightWeightMarshaller::decodeString here since we
			 * cannot know for sure whether all necessary bytes are already
			 * available)
			 *
			 */
	...
	public Ceylan::Network::MultiplexedProtocolBasedStreamServer
{
	...
	public:
			...
			throw( ServerStreamSocketException )
		{
			...
			/*
			 * Let's create the server-specific protocol manager which will use
			 * the specified marshaller to take care of lower-level data
			 * conversion:
			 *
			 */
			 ...
			 * owns the marshaller. Hence when the connection will be over, the
			 * connection-dedicated socket will be deallocated, which in turn
			 * will delete the protocol server, which in turn will delete the
			 * marshaller.
			 *
			 */
	...
	private:
		 ...
		 *
		 * As it is a state-less marshaller, only one instance can be used by a
		 * set of connections. If it was stateful, then each connection would
		 * have to be associated with its o
		 *
		 */
